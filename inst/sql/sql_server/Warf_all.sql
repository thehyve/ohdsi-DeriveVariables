-- Cohort for the Warfarin treatment group exported from ATLAS.
-- Last update: 26-10-2016
-- Drug exposure of Warfarin between October 3, 2012 and December, 31, 2014
-- Age Greater or equal to 18
-- Condition occurrence of atrial fibrillation (AF) before index date
-- No other anticoagulants before index date
-- Excluding first occurrence of AF after index
...
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (40241331,40244448,40244444,40241333,40241336)and invalid_reason is null
...
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (313217,4108832,4154290,4141360,4232697)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (43013024,43013025,43013030,43013026,43013033)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (43013024,43013025)
  and c.invalid_reason is null
...
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (45775372,40228164,40228152,40228160,40228153)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (45775372,40228164,40228152)
  and c.invalid_reason is null
...
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (1310149,40093132,40163533,40163553,40163534)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40093132)
  and c.invalid_reason is null
...
WHERE (C.drug_exposure_start_date >= TO_DATE(TO_CHAR(2012,'0000')||'-'||TO_CHAR( 10,'00')||'-'||TO_CHAR( 03,'00'), 'YYYY-MM-DD') and C.drug_exposure_start_date <= TO_DATE(TO_CHAR(2014,'0000')||'-'||TO_CHAR( 12,'00')||'-'||TO_CHAR( 31,'00'), 'YYYY-MM-DD'))
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + 0) and A.START_DATE <= P.OP_END_DATE
GROUP BY p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) <= 0
