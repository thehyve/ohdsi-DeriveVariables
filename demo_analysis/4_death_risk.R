...
ph.summary <- printSurvivalStatistics( daysAtRiskAll, is_deceased, RIVA_OR_VKA_STRING )
print(ph.summary)

# Proportional Hazards Regression with stratification for gender and age
ageAtIndex <- getAgeAtIndex( cohort )
ph.model.adjusted <- coxph( Surv(daysAtRiskAll,is_deceased) ~ RIVA_OR_VKA + strata(GENDER_STRING, ageAtIndex), cohort )
